...
#if defined(USE_PTC06)
    //Node mang gan sensor CMR su dung ham nay de gui du lieu ve Router-EMB
   void Send_IMAGE_ToRouterEmboard(WORD HTE_Cluster);
...
int main(void)
{
    ...
    /********trongkhanh******/
       ...
       CYCLE_SENDING_5M = 2*ONE_MINUTE_VALUE;// chu ky kiem tra du lieu là 5 phút
       CYCLE_SENDING = 5*ONE_MINUTE_VALUE;//chu ky gui du lieu dinh ky là 1 ngày 1 lan
                        ...
                        switch (params.APSDE_DATA_indication.ClusterId.Val)
                        {
                            //user clusterID application
                                ...
                                case HTE_REQUEST_CLUSTER://gui du lieu nhiet do do am ma emboard yeu cau
                                {
                                    ...
                                    if(((tempcurrent-tempere)>=5)||((tempere-tempcurrent)>=5)){
                                ...
                                break;
                            #endif
                        /****************lenh chup anh va gui du lieu ve R-EMB *****************/
                        /*quanghoa*/
                            #if defined(USE_PTC06)
                            ...
                            case CMR_REQUEST_CLUSTER:{
                                    data = APLGet();
                                    Take_photo();
                                    Send_IMAGE_ToRouterEmboard(CMR_RESPONSE_CLUSTER);
                                    #if defined(USE_DEBUG)
                                        printf("Just sent data for request of Router_EMB\r\n");
                                    #endif
                            }
        ...
        case NO_PRIMITIVE:
            ...
            else
            {
                if(!ZigBeeStatus.flags.bits.bHasBackgroundTasks)
                {
                    ...
                    // **************quanghoa********************
                    //so sanh thoa man nguong chenh lech nhiet do de gui dinh ki 
                            //tempcurrent=(float)(temperature*0.01-40.00);
                            //if(((tempcurrent-tempere)>=2)||((tempere-tempcurrent)>=2)){
                    ...
                    // **************quanghoa********************
                    #if defined(USE_PTC06)
                        //chup va gui data anh dinh ky ve router-emboard
                        if(ZigBeeReady() && WSANFlags.bits.CMR_Data_Ready)
                        {
                            WSANFlags.bits.CMR_Data_Ready = CLEAR;
                            Send_IMAGE_ToRouterEmboard(CMR_AUTO_SEND_CLUSTER);
                            #if defined(USE_DEBUG)
                                printf("Sending period data to RE \r\n");
                            #endif
                        }
...
/*********************************************************************
 * Function:        void Send_IMAGE_ToRouterEmboard(WORD HTE_Cluster)
 *
 * PreCondition:
 *
 * Cluster:         IMAGE_RESPONSE_CLUSTER
 *
 * Output:          None
 *
 * Side Effects:
 *
 * Overview:        Gui du lieu anh chup ve
 *                  router-EMB.
 *
 * Note:
 ********************************************************************/
void Take_photo(void){
    ...
    sendStartPhoto();
    sendGetLength();
    length_data=sendGetPhoto(data);
}
/***************ham lay du lieu va gui ban tin********/
